...
func main() {
	...
	collectdserver := flag.String("collectdserver", "localhost", "put here the collectd sevrer to send stats to")
	collectdport := flag.String("collectdport", "25826", "put here the collectd sevrer port to send stats to")
	hn, _ := os.Hostname()
	collectdhostname := flag.String("collectdhostname", hn, "If you want to spoof a hostname, put it here")
	imagesampleskip := flag.Int("sampleskip", 1, "if matchine is slow, make this a multiple of 2 to sample less of the image")
	flag.Parse()
